...
export function SeasonCalendar({
  ...
  return (
    <div ref={ref} className="relative w-full" data-testid={testId}>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-xl border border-border/60 p-4 w-[300px] animate-in fade-in-0 zoom-in-95 duration-150" data-testid={testId ? `${testId}-dropdown` : undefined}>
          <div className="flex items-center justify-between mb-3">
            <button
              type="button"
              onClick={prev}
              disabled={!canPrev()}
              className="p-1.5 rounded-lg hover:bg-secondary/60 disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`h-9 w-full rounded-lg text-sm font-medium transition-all select-none
