...
export const ROOM_DATA: Record<RoomCategory, RoomInfo> = {
  ...
  "Стандарт семейный с балконом": {
    ...
    description: "Однокомнатный номер площадью 20 м², подходящий для комфортного размещения до трёх гостей. В номере предусмотрена удобная зона отдыха с комфортным диваном и выход на балкон, создающий дополнительное пространство для отдыха.",
